...
// #ifdef POLYSOLVE_WITH_HYPRE
// TEST_CASE("hypre_Serena", "[solver]")
// {
//     const std::string path = "/home/yiwei/polyfem-data/Serena.mtx";
//     Eigen::SparseMatrix<double> A;
//     loadSymmetric(A, path);
//     Eigen::VectorXd b(A.rows());
//     b.setOnes();
//     Eigen::VectorXd x(b.size());
//     x.setZero();
...
//         end = omp_get_wtime();
//         solver->getInfo(solver_info);
//         std::cout << "Scalar Running time is " << double(end - start)  << std::endl;
//         std::cout << solver_info["num_iterations"] << std::endl;
//         std::cout << solver_info["final_res_norm"] << std::endl;
//     }
//     {
...
//         end = omp_get_wtime();
//         solver->getInfo(solver_info);
//         std::cout << "Block Running time is " << double(end - start) << std::endl;
//         std::cout << solver_info["num_iterations"] << std::endl;
//         std::cout << solver_info["final_res_norm"] << std::endl;
//     }
//     const double err = (A * x - b).norm() / b.norm();
...
TEST_CASE("catamari_smallscale", "[solver]")
{
    const std::string path = POLYSOLVE_DATA_DIR;
    Eigen::SparseMatrix<double> A;
    const bool ok = loadMarket(A, path + "/A_2.mat");
    ...
    x.setZero();
    {
        ...
        json solver_info;
        start = clock();
        auto solver = LinearSolver::create("Catamari", "");
        json params;
        solver->setParameters(params);
        solver->analyzePattern(A, A.rows());
        solver->factorize(A);
...
TEST_CASE("catamari_Serena", "[solver]")
{
    const std::string path = "/home/yiwei/polyfem-data";
    Eigen::SparseMatrix<double> A;
    loadSymmetric(A, path + "/Serena.mtx");
    Eigen::VectorXd b(A.rows());
    b.setOnes();
    Eigen::VectorXd x(b.size());
    x.setZero();
